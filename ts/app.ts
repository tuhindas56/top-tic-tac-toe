;(function () {
  // Game logic
  const gameBoard = (() => {
    let board: [(string | number)[], (string | number)[], (string | number)[]] = [
      [0, 1, 2],
      [3, 4, 5],
      [6, 7, 8],
    ]
    ...
    function updateBoard(position: number[], marker: string) {
      if (
        !(marker === "X" || marker === "O") ||
        position[0] > 3 ||
        position[0] < 1 ||
  ...
  const gameController = (() => {
    ...
    function setPlayerNames(name1: string, name2: string) {
  ...
  const displayController = (() => {
    // Cache DOM
    const playerOneName = document.querySelector('input[name="firstPlayer"]') as HTMLInputElement
    const playerTwoName = document.querySelector('input[name="secondPlayer"]') as HTMLInputElement
    const dialog = document.querySelector("dialog") as HTMLDialogElement
    const playButton = document.querySelector('button[type="submit"]') as HTMLButtonElement
    const grid = document.querySelector(".grid") as HTMLDivElement
    const gridItems = document.querySelectorAll(".grid > div") as NodeList
    ...
    function getPlayerNames(event: Event) {
    ...
    function updateText(target: HTMLElement | Node, string: string) {
    ...
    function gameOutcomeDisplayHandler() {
      if (
        gameBoard.evaluateBoard() !== undefined ||
